...
export default function CategoryPage({ params }: CategoryPageProps) {
  const categorySlug = useMemo(() => params.slug, [params.slug]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        // 2) 解析为 report 列表（只包含 path/title）
  ...
  const getCategoryInfo = (slug: string): ReportCategory => {
    return REPORT_CATEGORIES.find((cat) => cat.slug === slug) || REPORT_CATEGORIES[0];
  };

  const getCategoryColor = (slug: string) => {
    return CATEGORY_COLORS[slug] || CATEGORY_COLORS['shi-zheng-yu-guo-ji'];
  };
